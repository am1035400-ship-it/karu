            ...
            if ($cekUser) {
                ...
                if (password_verify($hashPassword, $cekUser->password)) {
                    ...
                    $data = [
                        ...
                        'time_login' => $stay_log ? (new Time('now'))->addHours(24) : (new Time('now'))->addMinutes(30),
                        'time_since' => $now,
                    ];

                    include('conn.php');

                    $sql = "SELECT `expiration_date` FROM `users` WHERE `username` = '$usernam' AND `expiration_date` > '" . $now->toDateTimeString() . "'";
        ...
        if (!$this->validate($form_rules)) {
            ...
            if (!$rCheck) {
                ...
                if ($rCheck->used_by) {
                    ...
                    $query = mysqli_query($conn, $sql);
                    $period = mysqli_fetch_assoc($query)['acc_expiration'];

                    $sql1 = "SELECT `level` FROM `referral_code` WHERE `Referral` = '$referral'";
                    $query1 = mysqli_query($conn, $sql1);
                    $userLevel = mysqli_fetch_assoc($query1)['level'];
